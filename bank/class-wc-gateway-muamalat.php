...
class WC_Gateway_Muamalat extends WC_Gateway_BACS {
	function __construct() {
		$this->id = "bank_muamalat";
		$this->method_title = __( "Bank Muamalat", 'woocommerce' );
		$this->method_description = __( "Pembayaran melalui Bank Muamalat", 'woocommerce' );
		$this->title = __( "Transfer Bank Muamalat", 'woocommerce' );
        $this->icon = $this->enable_icon = 'yes' === $this->get_option( 'enable_icon' ) ? plugins_url('assets/logo-muamalat.png',__FILE__) : null;
 ...
 public function init_form_fields() {
    $this->form_fields = array(
        'enabled' => array(
            ...
            'label'     => __( 'Enable <strong>Bank Muamalat</strong> Transfer Payment Method', 'woocommerce' ),
            'type'      => 'checkbox',
            'default'   => 'no',
        ),
        'title' => array(
...
public function email_instructions( $order, $sent_to_admin, $plain_text = false ) {
    ...
    if ( ! $sent_to_admin && 'bank_muamalat' === $order->get_payment_method() && $order->has_status( 'on-hold' ) ) {
        if ( $this->instructions ) {
