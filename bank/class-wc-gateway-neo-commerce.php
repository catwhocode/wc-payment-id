...
class WC_Gateway_Neo_Commerce extends WC_Gateway_BACS {
	function __construct() {
		$this->id = "bank_neo_commerce";
		$this->method_title = __( "Bank Neo Commerce", 'woocommerce' );
		$this->method_description = __( "Pembayaran melalui Bank Neo Commerce", 'woocommerce' );
		$this->title = __( "Transfer Bank Neo Commerce", 'woocommerce' );
        $this->icon = $this->enable_icon = 'yes' === $this->get_option( 'enable_icon' ) ? plugins_url('assets/logo-neo-commerce.png',__FILE__) : null;
    ...
    add_action( 'woocommerce_thankyou_bank_neo_commerce', array( $this, 'thankyou_page' ) );
    add_action( 'woocommerce_email_before_order_table', array( $this, 'email_instructions' ), 10, 3 );
     

    // BACS account fields shown on the thanks page and in emails
     $this->account_details = get_option( 'woocommerce_bank_neo_commerce_accounts',
        array(
            array(
 ...
 public function init_form_fields() {
    $this->form_fields = array(
        'enabled' => array(
            ...
            'label'     => __( 'Enable <strong>Bank Neo Commerce</strong> Transfer Payment Method', 'woocommerce' ),
            'type'      => 'checkbox',
            'default'   => 'no',
        ),
        'title' => array(
            ...
            'default'   => __( 'Transfer Bank Neo Commerce', 'woocommerce' ),
        ),
        'enable_icon' => array(
...
public function process_payment( $order_id ) {
    ...
    $order->update_status( 'on-hold', __( 'Menunggu Pembayaran melalui Bank Neo Commerce', 'woocommerce' ) );
...
public function email_instructions( $order, $sent_to_admin, $plain_text = false ) {
    ...
    if ( ! $sent_to_admin && 'bank_neo_commerce' === $order->get_payment_method() && $order->has_status( 'on-hold' ) ) {
        if ( $this->instructions ) {
