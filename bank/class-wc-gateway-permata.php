...
class WC_Gateway_Permata extends WC_Gateway_BACS {
	function __construct() {
		$this->id = "bank_permata";
		$this->method_title = __( "Bank Permata", 'woocommerce' );
		$this->method_description = __( "Pembayaran melalui Bank Permata", 'woocommerce' );
		$this->title = __( "Transfer Bank Permata", 'woocommerce' );
        $this->icon = $this->enable_icon = 'yes' === $this->get_option( 'enable_icon' ) ? plugins_url('assets/logo-permata.png',__FILE__) : null;
 ...
 public function init_form_fields() {
    $this->form_fields = array(
        'enabled' => array(
            ...
            'label'     => __( 'Enable <strong>Bank Permata</strong> Transfer Payment Method', 'woocommerce' ),
            'type'      => 'checkbox',
            'default'   => 'no',
        ),
        'title' => array(
...
public function email_instructions( $order, $sent_to_admin, $plain_text = false ) {
    ...
    if ( ! $sent_to_admin && 'bank_permata' === $order->get_payment_method() && $order->has_status( 'on-hold' ) ) {
        if ( $this->instructions ) {
