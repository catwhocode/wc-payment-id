...
class WC_Gateway_Mandiri extends WC_Gateway_BACS {
	function __construct() {
		...
		$this->method_description = __( "Pembayaran melalui Bank Mandiri", 'woocommerce' );
		$this->title = __( "Transfer Bank Mandiri", 'woocommerce' );
        $this->icon = $this->enable_icon = 'yes' === $this->get_option( 'enable_icon' ) ? plugins_url('assets/logo-mandiri.png',__FILE__) : null;
...
public function email_instructions( $order, $sent_to_admin, $plain_text = false ) {
    ...
    if ( ! $sent_to_admin && 'bank_mandiri' === $order->get_payment_method() && $order->has_status( 'on-hold' ) ) {
        if ( $this->instructions ) {
